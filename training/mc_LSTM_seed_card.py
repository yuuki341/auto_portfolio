...
class mcLSTM:
    ...
    def train(self):
        ...
        for seed in range(seed_num):
            ...
            #モデルの訓練・検証
            hist = model.fit(train_x, train_y, epochs=nasdaq_epoch[seed], validation_data=(val_x, val_y), batch_size=self.batch_size,verbose=0)
            ...
            #fig_name = f"test11/{nasdaq_epoch[seed]}_{args.m}_dropr{args.drop_ratio}_unit{args.u}_{seed}_fig.png"
            #plt.savefig(fig_name)
            val_graph = [np.min(hist.history['val_loss']),np.argmin(hist.history['val_loss'])]
            #txt_name = f"test11/{nasdaq_epoch[seed]}_{args.m}_dropr{args.drop_ratio}_unit{args.u}_{seed}.txt"
        ...
        #    np.savetxt(f"mcdrop_prediction/mc_drop_list_{day}.csv", mc_drop_list[day], delimiter=',')
        #np.savetxt("test_y.csv", test_y, delimiter=',')
        #np.savetxt("test_y_pred.csv", test_y_pred, delimiter=',')
        ...
        np.savetxt(f'./test11/{args.f}_seed{args.seed}_epoch{args.e}_{args.m}_index.csv', top_n_list_index_test_days, delimiter=',')
        np.savetxt(f'./test11/{args.f}_seed{args.seed}_epoch{args.e}_{args.m}_weight.csv', day_weight, delimiter=',')
        print(exam)
