...
class mcLSTM:
    ...
    def train(self):
        ...
        train_x = train_x[p]
        train_y = train_y[p]
        mc_drop_list = np.zeros([test_y.shape[0], len(self.tickers), 5],dtype=float)
        ...
        for seed in range(seed_num):
            ...
            #fig_name = f"test12/{nasdaq_epoch[seed]}_{args.m}_dropr{args.drop_ratio}_unit{args.u}_{seed}_fig.png"
            #plt.savefig(fig_name)
            val_graph = [np.min(hist.history['val_loss']),np.argmin(hist.history['val_loss'])]
            #txt_name = f"test12/{nasdaq_epoch[seed]}_{args.m}_dropr{args.drop_ratio}_unit{args.u}_{seed}.txt"
        ...
        for day in range(test_y.shape[0]):
            pred= np.mean(mc_drop_list[0],axis=1)
            print(mean_squared_error(pred ,test_y[0]))
