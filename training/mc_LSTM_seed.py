...
class mcLSTM:
    ...
    def train(self):
        ...
        for seed in range(seed_num):
            ...
            #モデルの訓練・検証
            hist = model.fit(train_x, train_y, epochs=nyse_epoch[seed], validation_data=(val_x, val_y), batch_size=self.batch_size,verbose=0)
            ...
            fig_name = f"test10/{nyse_epoch[seed]}_{args.m}_dropr{args.drop_ratio}_unit{args.u}_{seed}_fig.png"
            plt.savefig(fig_name)
            val_graph = [np.min(hist.history['val_loss']),np.argmin(hist.history['val_loss'])]
            txt_name = f"test10/{nyse_epoch[seed]}_{args.m}_dropr{args.drop_ratio}_unit{args.u}_{seed}.txt"
        ...
        for day in range(test_y.shape[0]):
            ...
            if self.top_num != 1:
                #top_n_list_index_test_days[day,:], day_weight[day,:] = greedy(self.top_num, mc_drop_list[day], self.mcdrop_num * seed_num)
        ...
        np.savetxt(f'./test10/{args.f}_seed{args.seed}_epoch{args.e}_{args.m}_index.csv', top_n_list_index_test_days, delimiter=',')
        np.savetxt(f'./test10/{args.f}_seed{args.seed}_epoch{args.e}_{args.f}_weight.csv', day_weight, delimiter=',')
