...
def optimization(test_y, test_y_pred, top_num, mc_drop_list, mcdrop_num, fname):
    ...
    for day in range(test_y.shape[0]):
        if self.top_num != 1:
            ...
            day_weight[day,:] = 1
    evaluate_portfolio(test_y_pred, test_y, top_n_list_index_test_days, day_weight, fname=fname)
    np.savetxt(f'./test6/{args.f}_seed{args.seed}_epoch{args.e}_{args.m}_index.csv', top_n_list_index_test_days, delimiter=',')
    np.savetxt(f'./test6/{args.f}_seed{args.seed}_epoch{args.e}_{args.f}_weight.csv', day_weight, delimiter=',')
